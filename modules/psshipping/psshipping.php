...
class Psshipping extends Module
{
    ...
    public function hookDisplayBackOfficeHeader(): string
    {
        if (
            ...
            !empty($this->context->controller->controller_name) &&
            $this->context->controller->controller_name === 'AdminCarriers'
        ) {
            Media::addJsDef([
            ...
            return $this->display(__DIR__, '/views/templates/hook/psshippingPromote.tpl');
        }
     ...
     * @return string
     */
    public function getCloudsyncCdcUrl(): string
    {
        /** @var ConfigHelper $config */
        $config = $this->getService('psshipping.helper.config');

        return $config->cloudsync_cdc_url;
    }
    ...
    {
        /** @var Carrier $shipment */
        $shipment = $params['carrier'];

        if (!in_array($shipment->external_module_name, CarrierService::CARRIERS_CREATED_BY_MODULE)) {
            return;
        }

        if ((bool) $shipment->active === true) {
            $segment = new Segment($this);
            $segment->setMessage('[SHI] Carrier Enabled');
            $segment->setOptions([
                'date' => date('Ymd'),
                'enabled' => true,
                'carrier_name' => $shipment->name,
                'mbe_shipping_service' => explode('_', $shipment->external_module_name)[1],
            ]);
            $segment->track();
        }
    }
    ...
    public function renderOrderDetail(Order $order): array
    {
        ...
        return [
            ...
            'deliveryAddress' => [
                ...
                'shipment' => $this->buildShipmentDetail($order, $carrier->external_module_name),
            ],
     ...
     */
    private function buildShipmentDetail($order, $carrierName)
    {
        $configuration = new Configuration();
        $weight = 0;
        $width = 0;
    ...
    private function getAssetsPath(): array
    {
        ...
        if (!$this->isDevEnvironment()) {
            ...
            $pathAppBuilded = $this->getPathUri() . '/views/build/js/psshipping-web-component.js';
